  ...
  template: `
    <div class="pagecls">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <button *ngIf="canAddSanita" class="btn add-btn btn-sm" (click)="modal.openModal()">{{ 'sanita.actions.addEvent' | translate }}</button>
      </div>
...
export class SanitaListComponent implements OnInit, OnDestroy {
  ...
  canAddSanita = false;
  editPermission = PERMISSIONS.SanitaEdit;
  deletePermission = PERMISSIONS.SanitaDelete;
  ...
  ngOnInit(): void {
    this.canAddSanita = this.commonService.checkPermission(PERMISSIONS.SanitaAdd, false);
    if (!this.commonService.checkPermission(PERMISSIONS.SanitaView, false)) {
      return;
    }
    this.load();
  }
  ...
  onDelete(row: any): void {
    if (!this.commonService.checkPermission(PERMISSIONS.SanitaDelete)) {
      return;
    }
